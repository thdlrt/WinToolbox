...
## 2026-09-26 · 0.3.0 统一 WebDAV

- Windows Python 完整套件 531 项通过；前端 11 组回归通过，TypeScript 与生产构建通过。
- 真实临时 Python 后端与 Edge：统一设置、连接测试、配置上传/列表/恢复、记账不被恢复回滚及中转站设置通过。
- 合成 WebDAV + 指定 Android 模拟器：469 次请求，Android 13 项检查及 Windows 8 类验收通过，包含旧目录复制、冲突保留、恢复保留业务数据、根目录切换和往返后新增文件迁移。
- Windows 目录测试包含旧根不可用后重试、空目录与文件冲突、同账号密码轮换及服务目录隔离。
- 测试使用独立数据和构造的账号，未操作用户实体手机或私人 NAS。
- 最终跨端记账回归：276 次真实 HTTP 请求，8 类场景通过，覆盖项目/账目/诊断/附件双向传输、并发合并与冲突解决、删除、项目日期导出和 Android 冷启动自动同步。
