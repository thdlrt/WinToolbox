...
$taskFFmpeg = (Get-Command ffmpeg.exe).Source
if ($taskFFmpeg -like '*scoop\shims*') { $taskFFmpeg = Join-Path $env:USERPROFILE 'scoop/apps/ffmpeg/current/bin/ffmpeg.exe' }
...
& $taskFFmpeg -L 2>&1 | Out-String | Set-Content -LiteralPath "$taskStage/tools/ffmpeg-license.txt" -Encoding utf8
& $taskFFmpeg -buildconf 2>&1 | Out-String | Set-Content -LiteralPath "$taskStage/tools/ffmpeg-buildconf.txt" -Encoding utf8
